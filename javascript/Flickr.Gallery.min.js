$(document).ready(function() {
	$("#gallery1").flickrGallery({
		PhotoSet : '72157669618659848',
	});
	$("#gallery2").flickrGallery({
		PhotoSet : '72157669618618688'
	});
	$("#gallery3").flickrGallery({
		PhotoSet : '72157691778490200'
	});
});
...
(function($) {
	...
	prty = {
		...
		settings : {
			...
			//FLICKR API KEY
			Key : 'e1a13c77b9a3adec4900d43875f98071',
			//Secret
			Secret : '85e8654fe48e3964',
			//FLICKR user ID
			User : '162122708@N03',
			//Flickr PhotoSet ID
			PhotoSet : '',
		...
		Setup : function(gDiv, settings) {
			...
			//prty.galleryDiv = $('#gallery1').append('<ul class="entries-columns"></ul>').find('ul');
			flickr = new Flickr({
		...
		buildThumb : function() {
			var loc;
			if (prty.ID == 72157669618659848) {
				//loc = '#gallery1';
				prty.galleryDiv = $('#gallery1').find('ul');

			} else if (prty.ID == 72157669618618688) {
				//loc = '#gallery2';
				prty.galleryDiv = $('#gallery2').find('ul');
			} else {
				prty.galleryDiv = $('#gallery3').find('ul');
			}
			var thisItem,
			    img;
			//Build ala-each fn
			for (var i = 0; i < prty.photoDetails.length; i++) {
				...
				//thisItem = $(loc).find('ul').append("<li id=" + i + "><a href='" + img.url_o + "'><img src='" + img.url_m + "' alt='" + img.title + "' /><div class='bg'></div><div class='content'></div><i class='show' data-href='" + img.url_o + "' data-photoid='" + img.id + "'>VIEW IT</i></a></li>").find('li .content').last();
				...
				//thisItem = document.getElementById(loc).getElementsByTagName('ul').appendChild("<li id=" + i + "><a href='" + img.url_o + "'><img src='" + img.url_m + "' alt='" + img.title + "' /><div class='bg'></div><div class='content'></div><i class='show' data-href='" + img.url_o + "' data-photoid='" + img.id + "'>VIEW IT</i></a></li>").find('li .content').last();

				//thisItem = ID.find('ul').append("<li id=" + i + "><a href='" + img.url_o + "'><img src='" + img.url_m + "' alt='" + img.title + "' /><div class='bg'></div><div class='content'></div><i class='show' data-href='" + img.url_o + "' data-photoid='" + img.id + "'>VIEW IT</i></a></li>").find('li .content').last();
			 ...
			 */
			$("#gallery1").find('li img').on('load', function() {
				$(this).parents('li').animate({
					opacity : 1.0
				}, 255);
			});
			$("#gallery2").find('li img').on('load', function() {
				$(this).parents('li').animate({
					opacity : 1.0
				}, 255);
			});
			$("#gallery3").find('li img').on('load', function() {
				$(this).parents('li').animate({
					opacity : 1.0
				}, 255);
			});
		...
		thumbHover : function(item, plusIconPos, titlePos, opacity) {
			...
			item.find('.bg').stop(true, false).animate({
				opacity : opacity
			}, 350, 'easeInOutCubic').parent().find('.show').stop(true, false).animate({
				top : plusIconPos,
				opacity : opacity
			}, 450, 'easeOutElastic').parent().find('.content').stop(true, false).animate({
				top : titlePos,
				opacity : opacity
			}, 450, 'easeOutElastic');
		},
		...
		viewBoxHTML : function(settings) {
			$('body').append('<div class="viewbox"><div class="panel"><h3>Title of Picture</h3><a href="#" class="resize icon"></a><a href="#" class="close icon"></a></div><div class="media_c"></div><div class="loader"></div></div>');
